...
class TestView(TestCase):
    def setUp(self):
        ...
        self.user_james = User.objects.create_user(
            username='james', password='dawk2022')
        self.user_kevin = User.objects.create_user(
            username='kevin', password='dawk2022')

        self.user_kevin.is_staff = True
        self.user_kevin.save()
    ...
    def test_create_post(self):
        ...
        # staff가 아닌 james가 로그인을 한다.
        self.client.login(username='james', password='dawk2022')
        response = self.client.get('/blog/create_post/')
        self.assertNotEqual(response.status_code, 200)

        # staff인 kevin로 로그인 한다.
        self.client.login(username='kevin', password='dawk2022')
        ...
        self.assertEqual(last_post.author.username, 'kevin')
    ...
    def test_comment_form(self):
        ...
        self.assertIn('kevin', new_comment_div.text)
        self.assertIn('KEVIN의 댓글입니다.', new_comment_div.text)

    def test_comment_update(self):
        comment_by_trump = Comment.objects.create(
            post=self.post_001,
            author=self.user_james,
            content='JAMES의 댓글입니다.'
        )
        ...
        self.assertIn('KEVIN의 댓글을 수정합니다.', comment_001_div.text)
        self.assertIn('Updated: ', comment_001_div.text)

    def test_delete_comment(self):
        comment_by_james = Comment.objects.create(
            post=self.post_001,
            author=self.user_james,
            content='JAMES의 댓글입니다.'
        )
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id='comment-area')
        self.assertNotIn('JAMES의 댓글입니다.', comment_area.text)
